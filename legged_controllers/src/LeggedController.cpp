...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw,
  ...
  std::vector<std::string> joint_names{"LF_HAA", "LF_HFE", "LF_KFE", "LH_HAA",
...
void LeggedController::update(const ros::Time& time,
  ...
  if (!safetyChecker_->check(currentObservation_, optimizedState,
                             optimizedInput)) {
    ROS_ERROR_STREAM(
  ...
  for (size_t j = 0;
       j < leggedInterface_->getCentroidalModelInfo().actuatedDofNum; ++j) {
    // Original kp: 0, kd: 3
    hybridJointHandles_[j].setCommand(posDes(j), velDes(j), 0.0, 3.0,
                                      torque(j));
  }
...
void LeggedController::updateStateEstimation(const ros::Time& time,
                                             const ros::Duration& period) {
  vector_t jointPos(hybridJointHandles_.size()),
      jointVel(hybridJointHandles_.size());
  ...
  for (size_t i = 0; i < 9; ++i) {
    ...
    angularVelCovariance(i) =
        imuSensorHandle_.getAngularVelocityCovariance()[i];
  ...
  stateEstimate_->updateImu(quat, angularVel, linearAccel,
                            orientationCovariance, angularVelCovariance,
                            linearAccelCovariance);
  measuredRbdState_ = stateEstimate_->update(time, period);
  currentObservation_.time += period.toSec();
  scalar_t yawLast = currentObservation_.state(9);
  currentObservation_.state =
      rbdConversions_->computeCentroidalStateFromRbdModel(measuredRbdState_);
...
LeggedController::~LeggedController() {
  ...
  std::cerr << "###############################################################"
               "#########";
  std::cerr << "\n### WBC Benchmarking";
  std::cerr << "\n###   Maximum : " << wbcTimer_.getMaxIntervalInMilliseconds()
            << "[ms].";
  std::cerr << "\n###   Average : " << wbcTimer_.getAverageInMilliseconds()
            << "[ms].";
}
...
void LeggedController::setupLeggedInterface(const std::string& taskFile,
                                            ...
                                            bool verbose) {
  leggedInterface_ =
      std::make_shared<LeggedInterface>(taskFile, urdfFile, referenceFile);
  leggedInterface_->setupOptimalControlProblem(taskFile, urdfFile,
                                               referenceFile, verbose);
}
...
void LeggedController::setupStateEstimate(const std::string& taskFile,
  ...
  stateEstimate_ = std::make_shared<KalmanFilterEstimate>(
      leggedInterface_->getPinocchioInterface(),
      leggedInterface_->getCentroidalModelInfo(), *eeKinematicsPtr_);
  dynamic_cast<KalmanFilterEstimate&>(*stateEstimate_)
      .loadSettings(taskFile, verbose);
...
PLUGINLIB_EXPORT_CLASS(legged::LeggedCheaterController,
                       controller_interface::ControllerBase)
