...
void LeggedController::starting(const ros::Time& time) {
  ...
  currentObservation_.state =
      rbdConversions_->computeCentroidalStateFromRbdModel(
          stateEstimate_->update(time, ros::Duration(0.002)));
...
void LeggedController::update(const ros::Time& time,
  ...
  currentObservation_.state =
      rbdConversions_->computeCentroidalStateFromRbdModel(measuredRbdState);
  ...
  vector_t x = wbc_->update(optimizedState, optimizedInput, measuredRbdState,
...
void LeggedController::setupStateEstimate(
    const std::string& /*urdfFile*/,
