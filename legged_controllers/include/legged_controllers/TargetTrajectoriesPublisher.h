...
class TargetTrajectoriesPublisher final {
 public:
  using CmdToTargetTrajectories = std::function<TargetTrajectories(
      const vector_t& cmd, const SystemObservation& observation)>;
  ...
  TargetTrajectoriesPublisher(
    ...
    targetTrajectoriesPublisher_.reset(
        new TargetTrajectoriesRosPublisher(nh, topicPrefix));
    ...
    auto cmdVelCallback = [this](const geometry_msgs::Twist::ConstPtr& msg) {
      if (latestObservation_.time == 0.0) {
        // std::cout << "Latest observation: " << latestObservation_.time
        //           << std::endl;
        return;
      }
    ...
    goalSub_ = nh.subscribe<geometry_msgs::PoseStamped>(
